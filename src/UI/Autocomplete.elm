...
autocomplete { view, events, render } =
    div [ class "dropdown w-full" ]
        [ div [ class "text-gray-dim p-2 w-full font-mono font-light flex items-center gap-2" ]
            [ input (events.inputEvents ++ [ value view.query, spellcheck False, autofocus True, placeholder "Type to search", class "bg-base-100 grow outline-none" ]) []
            , case view.status of
        ...
        , case view.status of
            ...
            Autocomplete.FetchedChoices ->
                case view.choices of
                    ...
                    _ ->
                        ul [ class "menu dropdown-content w-full bg-base-200 rounded shadow-2xl z-10" ] (List.indexedMap (render events.choiceEvents view.selectedIndex) view.choices)
        ]
